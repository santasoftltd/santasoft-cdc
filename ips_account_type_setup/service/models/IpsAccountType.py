...
class IpsAccountType(models.Model):
    id = models.AutoField(primary_key=True, db_column='ID')  # Auto increment starting at 1000 (can be set via migration)
    type = models.CharField(max_length=255, unique=True, db_column='Type')  # Unique string field

    class Meta:
        managed = False
        db_table = '[Account Type]'  # Define the table name
    ...
    def save(self, *args, **kwargs):
        super(IpsAccountType, self).save(*args, **kwargs)
        return self.id
