...
from .models.IpsAccountType import IpsAccountType
...
class API(APIView):
        ...
        try:
            ...
            if all(key in parameter for key in ('Filter', 'Sort', "Offset", "Limit", "Id")):
                if type(parameter['Offset']) is int and type(parameter['Limit']) is int and type(parameter['Id']) is int and type(parameter['Filter']) is str and type(parameter['Sort']) is str:
                    ...
                    if parameter['Id'] == 0:
                        response_data = IpsAccountType.get_all_objects()
                    else:
                        response_data = IpsAccountType.get_object_by_id(id=parameter['Id'])
            ...
            response = Response({
                'message': 'IPS Account type setup service failed. Error message: ' + error
    ...
    def update_type(self, request):
        try:
            data = request.data['Data']
            try:
                object = IpsAccountType.objects.get(id=data['id'])
            ...
            response = Response({
                'message': 'IPS Account type setup service failed. Error message: ' + str(error)
                }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        finally:
            return response
