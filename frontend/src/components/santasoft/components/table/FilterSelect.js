  ...
  return (
    <div>
      ...
      <select className='filterSelect-select' onChange={e => onAccessorSelect(e)} value={property.accessor}>
        <option key='' value=''>Select property</option>
...

      {
        ...
        <>
          ...
          {/* <select className='filterSelect-select' onChange={e => onValueSelect(e.target.value)} default=''>
            <option key='' value=''>Select value</option>
            
            {data.map((object) => (
              <option className='filterSelect-select-option' key={object['id']} value={object[property.accessor]}>{object[property.accessor]}</option>
            ))}
          </select> */}
        </>
      }
