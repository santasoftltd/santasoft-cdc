...
function CounterpartyGrid({ user, refresh, date, data, setData, setUserInput, addMessageHandler }) {
  ...
  const fetchAPI = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(ip + '/sfe05/57lLHq4yGGY5bpQFFL2azHQ32345bX&A6zY4GfAD7OeJYjk67!CFpJV//' + date + '/' + currentPage + '/' + pageSize + '/', {
        method: 'Get',
        headers: {
          ...
          <tbody>
            {
              data.map((object, index) => (
                <tr key={index} onDoubleClick={() => setUserInput(object)}>
                  ...
                  {columns[0].display && <td>{object["code"]}</td>}
                  {columns[1].display && <td>{object["name"]}</td>}
                  {columns[2].display && <td>{formatter(object["spot_limit"])}</td>}
                  {columns[3].display && <td>{formatter(object["forward_limit"])}</td>}
                </tr>
              ))
            }
          </tbody>
...
export default CounterpartyGrid