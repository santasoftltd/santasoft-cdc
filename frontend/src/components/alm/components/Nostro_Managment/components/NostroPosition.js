...
function NostroPosition({ user, refresh, date, addMessageHandler }) {
    ...
    const fetchAPI = async () => {
        try {
            setIsLoading(true);
            const response = await fetch(ip + '/sm01/zanYINzzOOts1kqFo$or4R6QIfqoBiCm@&ASVzcSmCzLqj9PW$/' + date + '/' + filter + '/' + sort + '/' + currentPage + '/' + pageSize + '/none/', {
                method: 'Get',
                headers: {
    ...
    const onDownloadButtonClicked = async () => {
        ...
        if (selectedRows.length) {
            ...
            try {
                setIsLoading(true);
                const response = await fetch(ip + '/sm01/zanYINzzOOts1kqFo$or4R6QIfqoBiCm@&ASVzcSmCzLqj9PW$/' + date + '/' + filter + '/' + sort + '/' + currentPage + '/' + pageSize + '/download/', {
                    method: 'Get',
                    headers: {
                        'Content-type': 'application/json; charset=UTF-8',
                        'Authorization': 'Token ' + user.token + ''
    ...
    return (
        <div className='table-container nostro-management-sub-home-second-child'>
          ...
          <div className='table-container-name'>
            <p style={{ display: 'inline' }}>Nostro Position</p>
            ...
            <div className='transaction-grid-button' onClick={() => saveClosingHandler()}>Save closing</div>
            {data.length !== 0
                    ? <div style={{ float: 'right', marginRight: '10px', marginTop: '0.3%' }}>Last date: {dateFormatter(data[0].date)}</div>
              ...
              <tbody>
                {
                  data.map((object) => (
                    <tr key={object["id"]}>
                      ...
                      {columns[0].display && <td>{object["account"]}</td>}
                      {columns[1].display && <td>{object["currency"]}</td>}
                      {columns[2].display && <td>{formatter(object["opening"])}</td>}
                      {columns[3].display && <td>{formatter(object["debit"])}</td>}
                      {columns[4].display && <td>{formatter(object["credit"])}</td>}
                      {columns[5].display && <td>{formatter(object["closing"])}</td>}
                    </tr>
                  ))
                }
              </tbody>
