...
function MonthlyDailyIncome({ user, refresh, year, setMonth, addMessageHandler }) {
    ...
    const fetchAPI = async () => {
        try {
            setIsLoading(true);
            const response = await fetch(ip + '/sfe05/rlJShXMqzOyKYEp9bokeYIgT@JhzvhhbaYZ!&XunHouVhnQh91/' + year + '/', {
                method: 'Get',
                headers: {
    ...
    return (
        <div className='table-container'>
            ...
            <div className='table-container-name' style={{ border: 'none' }}>
                <p style={{ display: 'inline' }}>Monthly cash flow - {year}</p>
...
export default MonthlyDailyIncome