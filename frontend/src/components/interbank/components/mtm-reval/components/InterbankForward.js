...
function InterbankForward({ user, refresh, date, currencyOne, currencyTwo, addMessageHandler }) {
    ...
    const fetchAPI = async () => {
        try {
            setIsLoading(true);
            const response = await fetch(ip + '/sfe05/WCfYb@p4WFqNCMjl6O!Si4bCYOH6NWlrB88wXYC8m$L*Cho&UP/' + filter + '/' + sort + '/empty/' + currentPage + '/' + pageSize + '/' + date + '/' + currencyOne + '/' + currencyTwo + '/', {
                method: 'Get',
                headers: {
    ...
    return (
        <div className='table-container'>
            ...
            {/* Table Title */}
            {/* <TableTitle title={'Interbank forward'} actions={actions} /> */}

            <div className='table-container-name'>
                <p style={{ display: 'inline' }}>Interbank forward</p>
...
export default InterbankForward