...
function NOP_PL({ user, refresh, date, addMessageHandler }) {
    ...
    const fetchAPI = async () => {
        try {
            setIsLoading(true);
            const response = await fetch(ip + '/sfe05/@QyW5aGrDjkd1FHqK7WNF1liE8o&52lX!duirIeKqcX3SEL62Y/' + date + '/', {
                method: 'Get',
                headers: {
    ...
    return (
        <div className='table-container'>
            ...
            {/* Table Title */}
            <TableTitle title={'NOP P&L'} actions={actions} />
                    ...
                    <tbody>
                        {
                            data.map((object, index) => (
                                <tr key={index}>
                                    ...
                                    {columns[1].display && <td>{formatter(object["nop_p_l"])}</td>}
                                </tr>
                            ))
                        }
                    </tbody>
