...
function SameDayMTM({ user, refresh, date, addMessageHandler }) {
    ...
    const fetchAPI = async () => {
        try {
            setIsLoading(true);
            const response = await fetch(ip + '/sfe05/YxZ!UM6ppo1HL7zfPzM*O&ii3xuha7j@sSUMePFQxqzNZe54IT/' + date + '/', {
                method: 'Get',
                headers: {
    ...
    return (
        <div className='table-container'>
            ...
            {/* Table Title */}
            <TableTitle title={'Same day MTM'} actions={actions} />
                    ...
                    <tbody>
                        {
                            data.map((object, index) => (
                                <tr key={index}>
                                    ...
                                    {columns[1].display && <td>{formatter(object["mtm"])}</td>}
                                </tr>
                            ))
                        }
                    </tbody>
