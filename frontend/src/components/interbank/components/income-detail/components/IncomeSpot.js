...
function IncomeSpot({ user, refresh, date, addMessageHandler }) {
    ...
    const fetchAPI = async () => {
        try {
            setIsLoading(true);
            const response = await fetch(ip + '/sfe05/L1Z24gaUA1LVuIG0xov$yTNkIahqKRbglHmVCRKHrmsR*o1RUy/' + date + '/', {
                method: 'Get',
                headers: {
    ...
    return (
        <div className='table-container'>
            ...
            {/* Table Title */}
            <TableTitle title={'Spot income'} actions={actions} />
