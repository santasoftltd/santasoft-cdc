...
function IncomeFwd({ user, refresh, date, addMessageHandler }) {
    ...
    const fetchAPI = async () => {
        try {
            setIsLoading(true);
            const response = await fetch(ip + '/sfe05/@uvLhACf9JqUB6umZs9xDbQn@P@bEQsIORSUKDe5ueSftAEQKk/' + date + '/', {
                method: 'Get',
                headers: {
    ...
    return (
        <div className='table-container'>
            ...
            {/* Table Title */}
            <TableTitle title={'Forward income'} actions={actions} />
