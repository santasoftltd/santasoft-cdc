...
function IncomeFwdSales({ user, refresh, date, addMessageHandler }) {
    ...
    const fetchAPI = async () => {
        try {
            setIsLoading(true);
            const response = await fetch(ip + '/sfe05/Vrf6u!kUa5@BwrTwx1WTL8YNBO0yGsLHpP$o5DuQ*qhgAesZTA/' + date + '/', {
                method: 'Get',
                headers: {
    ...
    return (
        <div className='table-container'>
            ...
            {/* Table Title */}
            <TableTitle title={'Forward sales income'} actions={actions} />
                    ...
                    <tbody>
                        {
                            data.map((object, index) => (
                                <tr key={index}>
                                    ...
                                    {columns[2].display && <td>{formatter(object["ccy_1"])}</td>}
                                    {columns[3].display && <td>{formatter(object["ccy_2"])}</td>}
                                    {columns[4].display && <td>{formatter(object["income"])}</td>}
                                </tr>
                            ))
                        }
                    </tbody>
