...
function YearlyDailyIncome({ user, refresh, year, month, monthNumber, addMessageHandler }) {
    ...
    const fetchAPI = async () => {
        try {
            setIsLoading(true);
            const response = await fetch(ip + '/sfe05/2TMNnoH4uwTHVGUD*zqldyI6LSW&19zzL*DeUWjUXB*fzrFVw$/' + year + '/' + monthNumber + '/', {
                method: 'Get',
                headers: {
    ...
    return (
        <div className='table-container'>
            ...
            <div className='table-container-name' style={{ border: 'none' }}>
                <p style={{ display: 'inline' }}>Yearly daily flow - {month} {year}</p>
                    ...
                    <tbody>
                        {
                            data.map((object, index) => (
                                <tr key={index}>
                                    ...
                                    {columns[15].display && <td>{formatter(object["day_total"])}</td>}
                                </tr>
                            ))
                        }
                    </tbody>
