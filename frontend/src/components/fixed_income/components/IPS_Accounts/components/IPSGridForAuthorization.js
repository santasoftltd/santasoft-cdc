...
function IPSGridForAuthorization({ user, setFloatForm, account, setUserInput, addMessageHandler }) {
    ...
    const fetchAPI = async () => {
        try {
            setIsLoading(true);
            const response = await fetch(ip + '/sm15/d3JXdnRtaloKFJcfEWp8Ih2hQU9cNCTq66vW5e5ge7so3uJz2h/approval/' + status + '/' + account + '/none/' + filter + '/' + sort + '/empty/' + currentPage + '/' + pageSize + '/', {
                method: 'Get',
                headers: {
    ...
    const submitHandler = async (id, action) => {
        try {
            ...
            setIsLoading(true);
            const response = await fetch(ip + '/sm15/d3JXdnRtaloKFJcfEWp8Ih2hQU9cNCTq66vW5e5ge7so3uJz2h/approval/none/none/none/none/none/empty/0/0/', {
                method: 'POST',
    ...
    const fetchByID = async (id) => {
        try {
            setIsLoading(true);
            const response = await fetch(ip + '/sm15/d3JXdnRtaloKFJcfEWp8Ih2hQU9cNCTq66vW5e5ge7so3uJz2h/updating/' + status + '/' + account + '/' + id + '/' + filter + '/' + sort + '/empty/' + currentPage + '/' + pageSize + '/', {
                method: 'Get',
                headers: {
                    'Content-type': 'application/json; charset=UTF-8',
                    'Authorization': 'Token ' + user.token + ''
    ...
    useEffect(() => {
        fetchAPI();
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [filter, sort, currentPage, pageSize, status, account]);
                        ...
                        <tbody>
                            {
                                data.map((object, index) => (
                                    <tr key={index}>
                                        ...
                                        {columns[14].display && <td>{object["last_modified_by"]}</td>}
                                        {columns[15].display && <td>{timestampFormatter(object["last_modified_date"])}</td>}
                                        {(columns[16].display && status === 'rejected') && <td>{object["rejected_by"]}</td>}
                                        {(columns[17].display && status === 'rejected') && <td>{timestampFormatter(object["rejected_date"])}</td>}
                                        {(columns[18].display && status === 'reversed') && <td>{object["reversed_by"]}</td>}
                                        {(columns[19].display && status === 'reversed') && <td>{timestampFormatter(object["reversed_date"])}</td>}
                                        {(columns[20].display && status === 'approved') && <td>{object["approved_by"]}</td>}
                                        {(columns[21].display && status === 'approved') && <td>{timestampFormatter(object["approved_date"])}</td>}
                                        {(columns[22].display && status === 'rejected') && <td>{object["rejection_reason"]}</td>}
                                        {(columns[23].display && status === 'reversed') && <td>{object["reversal_reason"]}</td>}
                                        {(columns[24].display && status === 'pending') && <td>
                                        ...
                                        {(columns[24].display && status === 'rejected') && <td>
                                            <div className='transaction-grid-multiple-button' style={{ backgroundColor: '#213af3' }} onClick={() => fetchByID(object["id"])}>Edit</div>
                                        </td>}
                                        {(columns[24].display && status === 'reversed') && <td>
                                            <div className='transaction-grid-multiple-button' style={{ backgroundColor: '#213af3' }} onClick={() => fetchByID(object["id"])}>Edit</div>
                                        </td>}
                                        {(columns[24].display && status === 'approved') && <td>
                                        ...
                                        {(columns[25].display && status === 'pending') && <td><input type="text" style={{ width: '250px' }} onChange={e => setReason(e.target.value)} value={reason} /></td>}
                                        {(columns[25].display && status === 'approved') && <td><input type="text" style={{ width: '250px' }} onChange={e => setReason(e.target.value)} value={reason} /></td>}
                                        {columns[26].display && <td style={{ cursor: 'pointer', textDecoration: 'underline' }}>view</td>}
                                    </tr>
                                ))
                            }
                        </tbody>
...
export default IPSGridForAuthorization