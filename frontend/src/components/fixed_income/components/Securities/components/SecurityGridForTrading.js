...
function SecurityGridForTrading({ user, securityType, setSecurityType, setSecurityFormClicked, addMessageHandler }) {
    ...
    const fetchAPI = async () => {
        try {
            setIsLoading(true);
            const response = await fetch(ip + '/sm15/FdirYW2qVPoIzLnbP07CW2adi1!0K2Mv*nTyUg5$7yWg*WwSQT/trading/none/' + securityType + '/none/none/none/none/0/0/', {
                method: 'Get',
                headers: {
                    ...
                    <tbody>
                        {
                            data.map((object, index) => (
                                <tr key={index}>
                                    ...
                                    {columns[7].display && <td><div className='transaction-grid-button' style={{backgroundColor:'MediumSeaGreen'}} onClick={() => setSecurityFormClicked(true)}>Trade</div></td>}
                                </tr>
                            ))
                        }
                    </tbody>
...
export default SecurityGridForTrading