...
function ForwardOutstanding({ user, refresh, date, onTableSelect, onForwardTransactionClicked, singleTitle, addMessageHandler }) {
  ...
  const fetchAPI = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(ip + '/sfe01/dsd1QYpNa5b$Ep8ZrAzZK9Vxh38qY3LGxsTUO28UrTC0y9*MgE/' + filter + '/' + sort + '/empty/' + date + '/' + currentPage + '/' + pageSize + '/', {
        method: 'Get',
        headers: {
      ...
      if (response.status === 200) {
        setData(result.forwardOutstanding);
        ...
        addMessageHandler({
          ...
          content: 'Forward outstanding grid failed to load due to unauthorized request.',
          type: 4
        })
      }
  ...
  const onDownloadButtonClicked = async () => {
    ...
    if (selectedRows.length) {
      ...
      try {
        setIsLoading(true);
        const response = await fetch(ip + '/sfe01/dsd1QYpNa5b$Ep8ZrAzZK9Vxh38qY3LGxsTUO28UrTC0y9*MgE/' + filter + '/' + sort + '/download/' + date + '/' + currentPage + '/' + pageSize + '/', {
          method: 'Get',
          headers: {
            'Content-type': 'application/json; charset=UTF-8',
            'Authorization': 'Token ' + user.token + ''
  ...
  return (
    <div className='table-container'>
      ...
      {/* Table Title */}
      {singleTitle ?
        <TableTitle title={'Forward outstanding'} />
        :
        <div className='table-container-name'>
          <select onChange={e => onTableSelect(e.target.value)} value='forward-outstanding'>
          ...
          <tbody>
            {
              data.map((object, index) => (
                <tr key={index}>
                  ...
                  {columns[11].display && <td>{object["days_at_risk_earned"]}</td>}
                  {columns[12].display && <td>{formatter(object["take_up_loss"])}</td>}
                  ...
                  {columns[19].display && <td>{formatter(object["amt_3rd_ccy_cross"])}</td>}
                  {columns[20].display && <td>{formatter(object["amt_i_b_prem"])}</td>}
                  {columns[21].display && <td>{formatter(object["amt_3rd_ccy_prem"])}</td>}
                </tr>
              ))
            }
          </tbody>
...
export default ForwardOutstanding