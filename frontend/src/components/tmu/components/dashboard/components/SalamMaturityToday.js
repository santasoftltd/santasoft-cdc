...
function SalamMaturityToday({ user, refresh, date, onTableSelect, onSalamTransactionClicked, singleTitle, addMessageHandler }) {
  ...
  const fetchAPI = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(ip + '/sfe01/CxAgojv$kNydOgC*pYb@LEhC1!lZeWU08jzeWd9L@RandwlBbb/' + filter + '/' + sort + '/empty/' + date + '/', {
        method: 'Get',
        headers: {
      ...
      if (response.status === 200) {
        ...
        setFetchedData(result.salamMaturityToday);
        ...
        addMessageHandler({
          ...
          content: 'Salam maturity grid failed to load due to unauthorized request.',
          type: 4
        })
      }
  ...
  const onDownloadButtonClicked = async () => {
    ...
    if (selectedRows.length) {
      ...
      try {
        setIsLoading(true);
        const response = await fetch(ip + '/sfe01/CxAgojv$kNydOgC*pYb@LEhC1!lZeWU08jzeWd9L@RandwlBbb/' + filter + '/' + sort + '/download/' + date + '/', {
          method: 'Get',
          headers: {
            'Content-type': 'application/json; charset=UTF-8',
            'Authorization': 'Token ' + user.token + ''
  ...
  return (
    <div className='table-container'>
      ...
      {/* Table Title */}
      {singleTitle ?
        <TableTitle title={'Salam maturity today'} />
        :
        <div className='table-container-name'>
          <select onChange={e => onTableSelect(e.target.value)} value='salam-maturity-today'>
          ...
          <tbody>
            {
              data.map((object, index) => (
                <tr key={index}>
                  ...
                  {columns[8].display && <td>{formatter(object["importt"])}</td>}
                  {columns[9].display && <td>{formatter(object["remaining_amount"])}</td>}
                  {columns[10].display && <td>{formatter(object["deal_rate"])}</td>}
                  {columns[11].display && <td>{formatter(object["i_b_rate"])}</td>}
                  {columns[12].display && <td>{object["branch"]}</td>}
                  {columns[13].display && <td>{dateFormatter(object["ttdate"])}</td>}
                  {columns[14].display && <td>{dateFormatter(object["vdate"])}</td>}
                  {columns[15].display && <td>{formatter(object["amt_3rd_ccy_cross"])}</td>}
                  {columns[16].display && <td>{formatter(object["amt_i_b_prem"])}</td>}
                  {columns[17].display && <td>{formatter(object["amt_3rd_ccy_prem"])}</td>}
                </tr>
              ))
            }
          </tbody>
...
export default SalamMaturityToday