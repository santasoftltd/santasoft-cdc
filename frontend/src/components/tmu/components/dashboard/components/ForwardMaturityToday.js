...
function ForwardMaturityToday({ user, refresh, date, onTableSelect, onForwardTransactionClicked, singleTitle, addMessageHandler }) {
  ...
  const fetchAPI = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(ip + '/sfe01/nvkcYzRXg1ZzuYGQPN@HEcrjd*ZILn6mTFIxUIw$Qm3Vx7Aljh/' + filter + '/' + sort + '/empty/' + date + '/', {
        method: 'Get',
        headers: {
  ...
  const onDownloadButtonClicked = async () => {
    ...
    if (selectedRows.length) {
      ...
      try {
        setIsLoading(true);
        const response = await fetch(ip + '/sfe01/nvkcYzRXg1ZzuYGQPN@HEcrjd*ZILn6mTFIxUIw$Qm3Vx7Aljh/' + filter + '/' + sort + '/download/' + date + '/', {
          method: 'Get',
          headers: {
            'Content-type': 'application/json; charset=UTF-8',
            'Authorization': 'Token ' + user.token + ''
  ...
  return (
    <div className='table-container'>
      ...
      {/* Table Title */}
      {singleTitle ?
        <TableTitle title={'Forward maturity today'} />
        :
        <div className='table-container-name'>
          <select onChange={e => onTableSelect(e.target.value)} value='forward-maturity-today'>
...
export default ForwardMaturityToday