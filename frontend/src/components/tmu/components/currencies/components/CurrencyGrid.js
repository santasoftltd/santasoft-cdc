...
function CurrencyGrid({ user, refresh, addMessageHandler }) {
  ...
  const fetchAPI = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(ip + '/sfe01/mhe1qRUeehtuqT6Z6B6z&crf0cJKU!8uya*FieK2XcoFioctX5/' + filter + '/' + sort + '/empty/', {
        method: 'Get',
        headers: {
      ...
      if (response.status === 200) {
        setFetchedData(result.currencies);
      }

      else if (response.status === 401) {
        addMessageHandler({
          title: 'Unable to load',
          content: 'Currencies grid failed to load due to unauthorized request.',
          type: 4
        })
      }
  ...
  const onDownloadButtonClicked = async () => {
    ...
    if (selectedRows.length) {
      ...
      try {
        setIsLoading(true);
        const response = await fetch(ip + '/sfe01/mhe1qRUeehtuqT6Z6B6z&crf0cJKU!8uya*FieK2XcoFioctX5/' + filter + '/' + sort + '/download/', {
          method: 'Get',
          headers: {
            'Content-type': 'application/json; charset=UTF-8',
            'Authorization': 'Token ' + user.token + ''
