...
function ProductGrid({ user, refresh, addMessageHandler }) {
  ...
  const fetchAPI = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(ip + '/sfe01/HrUT@3YDUM5w24TX98ED3s&roFpRlnqdZe&syXs!!EbHf7ywHs/' + filter + '/' + sort + '/empty/', {
        method: 'Get',
        headers: {
      ...
      if (response.status === 200) {
        ...
        addMessageHandler({
          ...
          content: 'Products grid failed to load due to unauthorized request.',
          type: 4
        })
      }
  ...
  const onDownloadButtonClicked = async () => {
    ...
    if (selectedRows.length) {
      ...
      try {
        setIsLoading(true);
        const response = await fetch(ip + '/sfe01/HrUT@3YDUM5w24TX98ED3s&roFpRlnqdZe&syXs!!EbHf7ywHs/' + filter + '/' + sort + '/download/', {
          method: 'Get',
          headers: {
            'Content-type': 'application/json; charset=UTF-8',
            'Authorization': 'Token ' + user.token + ''
          ...
          <tbody>
            {
              data.map((object, index) => (
                <tr key={index}>
                  ...
                  {columns[0].display && <td>{object["product"]}</td>}
                  {columns[1].display && <td>{object["type1"]}</td>}
                  {columns[2].display && <td>{object["type2"]}</td>}
                </tr>
              ))
            }
          </tbody>
