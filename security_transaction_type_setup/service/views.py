...
from .models.SecurityTransactionType import SecurityTransactionType
...
class API(APIView):
        ...
        try:
            ...
            if all(key in parameter for key in ('Filter', 'Sort', "Offset", "Limit", "Id")):
                if type(parameter['Offset']) is int and type(parameter['Limit']) is int and type(parameter['Id']) is int and type(parameter['Filter']) is str and type(parameter['Sort']) is str:
                    ...
                    if parameter['Id'] == 0:
                        response_data = SecurityTransactionType.get_all_objects()
                    else:
                        response_data = SecurityTransactionType.get_object_by_id(id=parameter['Id'])
            ...
            response = Response({
                'message': 'Security transaction type setup service failed. Error message: ' + error
    ...
    def add_type(self, request):
        try:
            ...
            object = SecurityTransactionType()
    ...
    def update_type(self, request):
        try:
            ...
            response = Response({
                'message': 'Security transaction type setup service failed. Error message: ' + str(error)
                }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        finally:
            return response
