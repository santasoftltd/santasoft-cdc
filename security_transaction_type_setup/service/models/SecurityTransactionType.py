...
class SecurityTransactionType(models.Model):
    id = models.AutoField(primary_key=True, db_column='Ano')  # Auto increment starting at 1000 (can be set via migration)
    type = models.CharField(max_length=50, db_column='Sale_Purchase')  # Unique string field
    type_id = models.IntegerField(db_column='ID')  # Unique string field

    class Meta:
        managed = False
        db_table = 'Sale_Purchase'  # Define the table name
    ...
    def save(self, *args, **kwargs):
        super(SecurityTransactionType, self).save(*args, **kwargs)
        return self.id
